...
void CrystalBallScene()
{
    ...
    std::ofstream out("refraction3.ppm");
    out << CV.ToPPM();
    out.close();
}
...
{
    Plane Pond(0);
    Pond.SetShadowOn(false);
    Pond.SetTransform(Matrix::Translation(0., 0., 0.));
    auto Mat = Material();
    Mat.SetColor(Color(0.8, 0.8, 0.9));
    Mat.SetSpecular(0.);
    Mat.SetReflective(0.75);
    ...
    Pond.SetMaterial(Mat);

    Plane Bottom(1);
    Bottom.SetTransform(Matrix::Translation(0., -2., 0.));
    Mat = Material();
    Mat.SetColor(Color(0.8, 0.8, 0.9));
    Mat.SetSpecular(0.);
    // Mat.SetReflective(0.75);
    ...
    Plane Wall(2);
    Wall.SetTransform(Matrix::RotationX(M_PI/2).Translate(0., 0., 50.));
    Mat = Material();
    Mat.SetColor(Color(0.8, 0.8, 0.9));
    Mat.SetSpecular(0.);
    // Mat.SetReflective(0.75);
